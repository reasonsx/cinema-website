...
function ratingBadge($rating){
    return $rating
        ? badge($rating, 'bg-indigo-100 text-indigo-800 border-indigo-200')
        : '<span class="text-xs text-gray-400 italic">N/A</span>';
}

function languageBadge($lang){
    return $lang
        ? badge($lang, 'bg-sky-100 text-sky-800 border-sky-200')
        : '<span class="text-xs text-gray-400 italic">N/A</span>';
}

function genreBadge($genre){
    return $genre
        ? badge($genre, 'bg-fuchsia-100 text-fuchsia-800 border-fuchsia-200')
        : '<span class="text-xs text-gray-400 italic">N/A</span>';
}

function moviePeopleBadges(PDO $db, int $movieId, string $type){
    $sql = $type === 'actor'
        ? "SELECT first_name,last_name FROM actors a JOIN actorAppearIn aa ON a.id=aa.actor_id WHERE aa.movie_id=?"
        : "SELECT first_name,last_name FROM directors d JOIN directorDirects dd ON d.id=dd.director_id WHERE dd.movie_id=?";
...
renderTable([
    ...
    'searchable' => true,
    'headers' => [
        'ID','Poster','Title','Year','Rating','Genre','Language','Length',
        'Description','Trailer','Actors','Directors'
    ],
    'rows' => $movies,
    ...
    'renderRow' => function ($movie) use ($db){
        ...
        // Poster
        $posterHtml = $movie['poster']
            ? "<img src='".e($movie['poster'])."' width='60' class='rounded shadow-sm'>"
            : "<span class='text-xs text-gray-400 italic'>No poster</span>";

        // Trailer
        $trailer = $movie['trailer_url']
            ? "<a href='".e($movie['trailer_url'])."' target='_blank' class='text-blue-600 hover:underline text-xs'>Open</a>"
    ...
    'actions' => function($movie){
        ...
        <div class="flex items-center gap-2">
            ...
            <!-- Delete -->
            <form method="post" onsubmit="return confirm('Delete movie?')" class="flex items-center justify-center p-0 m-0 leading-none">
    ...
    'renderEditRow' => function($movie) use ($db,$allActors,$allDirectors){
        ...
        <form method="post" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="md:col-span-2 flex flex-col">
                ...
                <textarea name="description" rows="3" class="input-edit-textarea"><?= e($movie['description']) ?></textarea>
            </div>
            ...
            <!-- People selection — simple checkboxes -->
            <div>
    ...
    /* ---------------------------------------------------------
       ADD MOVIE — Same simple checkbox UI
    --------------------------------------------------------- */
    'addLabel' => 'Add Movie',
