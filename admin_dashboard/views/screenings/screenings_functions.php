...
function hasScreeningConflict(PDO $db, int $roomId, string $start, string $end, ?int $excludeId = null): bool
{
    ...
    if ($excludeId) {
        ...
        $stmt->execute([
            $roomId, $start, $start, $end, $end, $start, $end, $excludeId
        ]);
    } else {
        $stmt = $db->prepare($query);
        $stmt->execute([
...
function addScreening(PDO $db, array $data): array
{
    try {
        ...
        $stmt = $db->prepare("
            INSERT INTO screenings (movie_id, screening_room_id, start_time, end_time)
            VALUES (?, ?, ?, ?)
        ");
        $stmt->execute([
            $data['movie_id'],
            $data['screening_room_id'],
            $data['start_time'],
            $data['end_time']
        ]);
...
function editScreening(PDO $db, array $data): array
{
    try {
        ...
        $stmt = $db->prepare("
            UPDATE screenings
            SET movie_id = ?, screening_room_id = ?, start_time = ?, end_time = ?
            WHERE id = ?
        ");
        $stmt->execute([
            $data['movie_id'],
